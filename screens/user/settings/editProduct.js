...
const EditProduct = ({ Product, openModal, setOpenModal }) => {
  // defines backendUrl
  ...
  const func = async () => {
    const filePath = `marketplace/${currentUser?._id}/products/images/${Product.gallery[0].folder}`;
  ...
  const ActiveProduct = async (value) => {
    try {
      setActive(value);
      await axios.patch(backendUrl + "/api/v1/marketplace/" + Product._id, {
        active: value,
      });
  ...
  const UploadProduct = async () => {
    ...
    let folderId = Product.gallery[0].folder;
    ...
    Promise.all(uploadPromises).then(async (uploadedUrls) => {
      try {
        ...
        await axios.patch(
          backendUrl + "/api/v1/marketplace/" + Product._id,
          product
        );
        dispatch(setRerenderProducts());
        setTimeout(() => {
  ...
  const DeleteProduct = async () => {
    try {
      ...
      await axios.delete(backendUrl + "/api/v1/marketplace/" + Product._id);
  ...
  return (
    ...
    >
      <KeyboardAvoidingView // <-- Add this wrapper
        style={{ flex: 1, backgroundColor: "rgba(1, 2, 12, 0.8)" }}
      ...
      >
        ...
        >
          <Pressable
            onPress={() => setOpenModal({ active: false, target: "" })}
            style={{
              width: "100%",
              alignItems: "center",
          ...
          >
              ...
              <View style={{ flexDirection: "row", gap: 15 }}>
                ...
                >
                  <Text
                    style={{
                      color:
                        sex === "all" ? currentTheme.pink : currentTheme.font,
                      letterSpacing: 0.3,
                    }}
                  >
                ...
                >
                  <Text
                    style={{
                      color:
                        sex === "men" ? currentTheme.pink : currentTheme.font,
                      letterSpacing: 0.3,
                    }}
                  >
            ...
            >
              ...
              <TextInput
                ...
                ref={saleRef}
                placeholder={
                  language?.language?.Marketplace?.marketplace?.addSale
                }
                placeholderTextColor={currentTheme.disabled}
                style={[
                  styles.inputField,
