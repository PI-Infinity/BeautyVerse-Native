...
export const FeedItem = () => {
      ...
      if (currentUser?._id !== user?._id) {
        await axios.post(
          backendUrl + `/api/v1/users/${user?._id}/notifications`,
          {
            ...
            type: "star",
            status: "unread",
            feed: `/api/v1/users/${user?._id}/feeds/${feedObj?._id}`,
          }
        );
      }
      setTimeout(() => {
      ...
      if (currentUser?._id !== user?._id) {
        await axios.post(
          backendUrl + `/api/v1/users/${user?._id}/notifications`,
          {
            ...
            type: "review",
            status: "unread",
            feed: `/api/v1/users/${user?._id}/feeds/${props?.feed?._id}`,
          }
        );
      }
      dispatch(setRerenderUserFeed());
  ...
  let addationalSize = feedObj.fileHeight - SCREEN_WIDTH;
  let inPercent = (addationalSize / feedObj.fileHeight) * 100;
  let height = feedObj.fileWidth - (feedObj.fileWidth / 100) * inPercent;
  ...
  if (feedObj.video) {
    let originalHeight =
      feedObj.fileWidth >= feedObj.fileHeight
        ? feedObj.fileWidth
        : feedObj.fileHeight;
    let originalWidth =
      feedObj.fileWidth >= feedObj.fileHeight
    ...
    let originalHeight = feedObj.fileHeight;
    let originalWidth = feedObj.fileWidth;
          ...
          {props?.feed?.fileFormat === "img" && (
            ...
            >
              <TopSection
                ...
                createdAt={feedObj.createdAt}
                DotsFunction={() => setFeedOption(!feedOption)}
                fileFormat={feedObj.fileFormat}
              />
            </View>
          )}
            ...
            {props?.feed?.fileFormat === "video" ? (
              <>
                ...
                <CacheableVideo
                  ...
                  style={{
                    height:
                      feedObj.fileHeight >= feedObj.fileWidth
                        ? hght
                        : feedObj.fileWidth,
                  }}
                  source={{
